...
public class S3SinkTask extends SinkTask {
  ...
  public void start(Map<String, String> props) throws ConnectException {
    ...
    valueConverter = Configure.buildConverter(config, "value.converter", false,
  ...
  public void put(Collection<SinkRecord> records) throws ConnectException {
    records.stream()
        ...
        .forEach((tp, rs) -> {
          ...
          log.debug("{} received {} records for {} to archive. Last offset {}", name(), rs.size(),
              tp,
              lastOffset);
  ...
  private String name() {
    return configGet("name").orElseThrow(
        () -> new IllegalWorkerStateException("Tasks always have names"));
  }
  ...
  private PartitionWriter initWriter(TopicPartition tp, long offset) {
    try {
      ...
      throw new RetriableException("Error initializing writer for " + tp + " at offset " + offset,
          e);
    }
  }
  ...
  private class PartitionWriter {
    ...
    private PartitionWriter(TopicPartition tp, long firstOffset) throws IOException {
      ...
      writer = new BlockGZIPFileWriter(name, path, firstOffset, GZIPChunkThreshold,
    ...
    private void writeAll(Collection<SinkRecord> records) {
      ...
      try (Metrics.StopTimer ignored = metrics.time("writeAll", tags)) {
        writer.write(format.writeBatch(records.stream()
            .map(record -> new ProducerRecord<>(record.topic(), record.kafkaPartition(),
                keyConverter.map(
                        c -> c.fromConnectData(record.topic(), record.keySchema(), record.key()))
                    .orElse(null),
