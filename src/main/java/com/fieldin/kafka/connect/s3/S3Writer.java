 ...
 * <p>
 * Maybe one day we could make this an interface and support pluggable storage backends... but for
 * now it's just to keep things simpler to test.
 */
public class S3Writer {
  ...
  public S3Writer(String bucket, String keyPrefix, AmazonS3 s3Client) {
    this(bucket, keyPrefix, s3Client,
  ...
  public long putChunk(String localDataFile, String localIndexFile, TopicPartition tp)
      throws IOException {
  ...
  public long fetchOffset(TopicPartition tp) throws IOException {
    ...
    try (
        S3Object cursorObj = s3Client.getObject(this.bucket,
            this.getTopicPartitionLastIndexFileKey(tp));
  ...
  private String getTopicPartitionLastIndexFileKey(TopicPartition tp) {
    return String.format("%slast_chunk_index.%s-%05d.txt", this.keyPrefix, tp.topic(),
        tp.partition());
  }
  ...
  private void updateCursorFile(String lastIndexFileKey, TopicPartition tp) throws IOException {
    try {
      ...
      s3Client.putObject(
          new PutObjectRequest(this.bucket, this.getTopicPartitionLastIndexFileKey(tp),
              contentsAsStream, md));
    } catch (Exception ex) {
